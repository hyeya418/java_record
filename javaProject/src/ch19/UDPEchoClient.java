...
public class UDPEchoClient {
	...
	public UDPEchoClient() {}//기본생성자
	
	public UDPEchoClient(String ip, int port) {
		try {
			InetAddress ia = InetAddress.getByName(ip);
			DatagramSocket ds = new DatagramSocket(port);
			System.out.print("서버측에서 보내는 메시지 : ");
			file = new BufferedReader(new InputStreamReader(System.in));
			str = file.readLine();
			//new String(바이트배열) : 바이트배열 ==> 문자열
			//문자열.getBytes() : 문자열 ==> 바이트 배열
			//문자열을 바이트배열로 변환(UDP통신에서는 바이트 배열로 전송해야함)
			byte buffer[] = str.getBytes();
			//데이터그램패킷 객체 생성(전송할 바이트 배열, 배열크기, ip주소, 서버포트 번호)
			...
			ds.close();//소켓 연결 닫기
			System.out.println("server ip : "+ dp.getAddress()+", server poty : "+dp.getPort());
